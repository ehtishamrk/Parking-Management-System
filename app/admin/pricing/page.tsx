...
export default function PricingPage() {
  ...
  async function load() {
    ...
    const [{ data: rulesData }, { data: settings }] = await Promise.all([
      ...
      supabase.from('lot_settings').select('payment_mode, pricing_mode').eq('id', 1).single(),
    ]);
    setRules(rulesData ?? []);
    setPaymentMode(settings?.payment_mode ?? 'both');
    setPricingMode(settings?.pricing_mode ?? 'hourly');
    setLoading(false);
  }
  ...
  async function savePricingMode(mode: 'hourly' | 'fixed') {
    setPricingMode(mode);
    await supabase.from('lot_settings').update({ pricing_mode: mode }).eq('id', 1);
    toast.success('Ticket pricing mode updated');
  }
  ...
  return (
    <div>
      ...
      <div className="bg-paper border border-steel-line rounded-xl p-5 mb-6">
        <p className="font-display text-lg mb-1">Ticket Pricing Mode</p>
        <p className="text-xs text-steel mb-3">Controls how regular tickets (not passes) are billed at the counter. Operators don't choose this — it's set here.</p>
        <div className="flex flex-wrap gap-2">
          <button
            onClick={() => savePricingMode('hourly')}
            className={`px-4 py-2 rounded-lg text-sm border ${
              pricingMode === 'hourly' ? 'bg-amber border-amber text-ink font-semibold' : 'border-steel-line text-steel'
            }`}
          >
            Hourly — billed on time parked, calculated at exit
          </button>
          <button
            onClick={() => savePricingMode('fixed')}
            className={`px-4 py-2 rounded-lg text-sm border ${
              pricingMode === 'fixed' ? 'bg-amber border-amber text-ink font-semibold' : 'border-steel-line text-steel'
            }`}
          >
            Fixed — flat rate per vehicle, charged at entry
          </button>
        </div>
      </div>
      ...
      <p className="text-xs text-steel mb-2">
        Hourly and Fixed rates apply to regular tickets (whichever mode is selected above). Daily, Monthly and Yearly rates are reference rates for issuing Passes.
      </p>
