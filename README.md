...
**Already ran schema.sql before?** Run these two migrations instead, in order:
- `supabase/migrations/002_billing_and_pricing_mode.sql` — adds admin-controlled
  pricing mode and the platform billing table.
- `supabase/migrations/003_data_retention_cron.sql` — optional, schedules
  the 3-month data purge (needs the `pg_cron` extension enabled first;
