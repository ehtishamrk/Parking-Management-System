...
create table lot_settings (
  ...
  pricing_mode text not null default 'hourly' check (pricing_mode in ('hourly', 'fixed')), -- admin chooses; governs the New Ticket counter flow (passes keep their own basis)
...
-- ---------- PLATFORM BILLING (what this lot owes the platform) ----------
-- Amount owed for a period ('YYYY-MM') = (tickets + passes created that
-- month) × 3.5, minus sum of platform_payments.amount for that period.
...
create index idx_platform_payments_period on platform_payments(period);

-- ---------- REVENUE LEDGER (permanent — never touched by the retention purge) ----------
-- One row per calendar day. Kept up to date automatically by triggers below,
-- so dashboard/billing revenue survives even after old tickets are purged.
...
create function ledger_on_ticket_insert() returns trigger as $$
begin
...
-- Hourly tickets get their final amount at exit (an update, not an insert) —
-- apply just the difference so the ledger stays correct either way.
create function ledger_on_ticket_amount_update() returns trigger as $$
begin
...
create function ledger_on_pass_insert() returns trigger as $$
begin
...
-- platform_payments: admins can view payment history. No insert/update policy
-- on purpose — payments are recorded by the platform operator directly in
-- SQL until a payment gateway is wired up, so the tenant's own admin can't
-- self-mark their bill as paid and bypass the lockout below.
create policy "platform_payments_admin_read" on platform_payments for select using (is_admin());

-- revenue_ledger: any signed-in staff can read; nobody writes it by hand,
-- it's maintained entirely by the triggers above.
...
-- ---------- DATA RETENTION (keep ticket records 3 months, then purge) ----------
-- Deletes only tickets older than 3 months — the "3-month customer backup."
-- Passes are never auto-deleted (a lapsed monthly/yearly pass is still
-- meaningful), and revenue_ledger above is never touched, so all-time
-- dashboard/billing numbers stay accurate forever regardless of what's
-- purged here. Not scheduled automatically — see
-- supabase/migrations/003_data_retention_cron.sql to turn on the daily
-- cron job once pg_cron is enabled for this project.
create function purge_old_records() returns void as $$
begin
