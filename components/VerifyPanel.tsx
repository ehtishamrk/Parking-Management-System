...
export default function VerifyPanel() {
  ...
  async function markExit() {
    ...
    await supabase
      .from('tickets')
      .update({ status: 'closed', exit_time: new Date().toISOString(), verified_at: new Date().toISOString(), verified_by: userData.user?.id })
    ...
    <div className="max-w-lg mx-auto">
      ...
      {result?.kind === 'ticket' && (
        <div className="mt-4 bg-paper border border-steel-line rounded-xl p-5">
          ...
          <Row label={t('amount')} value={formatCurrency(result.data.amount)} />
