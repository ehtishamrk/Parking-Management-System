...
export default function NewTicketForm() {
  ...
  const [basis, setBasis] = useState<PricingBasis>('hourly');
  ...
  if (issuedTicket && settings) {
    return (
      <div>
        <TicketReceipt ticket={issuedTicket} settings={settings} />
        <button
    ...
    <div className="max-w-lg mx-auto">
      ...
      {mode === 'pass' ? (
        ...
        <div className="bg-paper border border-steel-line rounded-xl p-5 space-y-5">
          ...
          <div>
            <label className="text-xs text-steel">Billing Basis</label>
            <select value={basis} onChange={(e) => setBasis(e.target.value as PricingBasis)} className="w-full mt-1.5 border border-steel-line rounded-lg px-3 py-2.5 outline-none focus:border-amber">
              {(['hourly', 'fixed', 'daily', 'monthly', 'yearly'] as PricingBasis[]).map((b) => (
                <option key={b} value={b}>{t(b)} — Rs {rules.find((r) => r.vehicle_type === vehicleType && r.basis === b)?.rate ?? '—'}</option>
              ))}
            </select>
          </div>
          ...
          <div className="flex items-center justify-between pt-2 border-t border-steel-line">
            ...
            <span className="font-display text-2xl">Rs {rate || 0}</span>
          </div>
