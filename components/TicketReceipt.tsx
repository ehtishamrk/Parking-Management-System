...
export default function TicketReceipt({ ticket, settings, autoPrint = false }: { ticket: Ticket; settings: LotSettings; autoPrint?: boolean }) {
  ...
  useEffect(() => {
    if (autoPrint && qrDataUrl) {
      // Small delay so the QR image is painted before the print snapshot is taken.
